...
const resolve = (absWorkingDir, b, p, options = {}) => {
  ...
  if (options.fallbackToEsbuild) {
    return (async () => await esbuildResolve(absWorkingDir, p, b))()
  }
...
async function esbuildResolve(absWorkingDir, id, resolveDir) {
  const esbuild = require(require.resolve('esbuild', { paths: [absWorkingDir] }))
